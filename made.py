...
class MADE(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.L = kwargs['L']
        self.n = self.L*args.L1  # Number of sites
    ...
    def forward(self, x):
        ...
        x_hat = self.net(x)
        x_hat = x_hat.view(x_hat.shape[0], 1, self.L, args.L1)
        ...
        return x_hat.to(default_dtype_torch)
    ...
    def sample(self, batch_size):
        ...
        sample[Record, 0, self.L - 1, args.L1 - 1] = 1
        # all-0 state: end
        '''
    ...
    def forward2(self, x):
        xx = torch.reshape(x, (x.shape[0] * x.shape[1], self.L * args.L1))  # x.view(x.shape[0]*x.shape[1], -1)
        x_hat = self.net(xx)
        x_hat = x_hat.view(x.shape[0], x.shape[1], self.L, args.L1)  # My code
        return x_hat
