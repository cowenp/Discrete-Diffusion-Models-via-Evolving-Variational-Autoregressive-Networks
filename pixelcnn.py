...
class PixelCNN(BaseModel):
    ...
    def sampleIS(self, batch_size):
        ...
        """
        sample = torch.zeros([batch_size, 1, self.L, args.L1], dtype=default_dtype_torch, device=self.device)
        binomialP = self.binomialP * torch.ones(sample.shape, dtype=torch.float64).to(self.device)
        x_hat = []
        for i in range(self.L):
            for j in range(args.L1):
    ...
    def sample(self, batch_size):
        ...
        Returns:
            Samples from the PixelCNN model
        """
        sample = torch.zeros([batch_size, 1, self.L, args.L1], device=self.device)

        for i in range(self.L):
            for j in range(args.L1):
                # print(f"s:{sample.dtype}")
                x_hat = self._forward(sample)  # Batch X 1 X L X L
    ...
    def denoise_sample(self, batch_size, noise_sample):
        """Sample method
        ...
        """
        sample = torch.zeros([batch_size, 1, self.L, args.L1], device=self.device)
        x_hat_new = torch.zeros([batch_size, 1, self.L, args.L1], device=self.device)
        weight = (1 - args.flip_p) * noise_sample + args.flip_p * (1 - noise_sample)
        ...
        for i in range(self.L):
            for j in range(args.L1):
                ...
                norm_Z = x_hat[:, :, i, j] * weight[:, :, i, j] + (1 - x_hat[:, :, i, j]) * (1 - weight[:, :, i, j])
                #print(norm_Z)
                x_hat_new[:, :, i, j] = (x_hat[:, :, i, j] * weight[:, :, i, j]) / norm_Z
                #print(x_hat_new)
                sample[:, :, i, j] = torch.bernoulli(x_hat_new[:, :, i, j]).to(default_dtype_torch) * 2 - 1
        # # all-0 state: begin
        # mask = (sample + 1) / 2
        # aa = torch.sum(torch.sum(torch.sum(mask, 3), 2), 1)
        # Record = aa == 0  # find all-0 configuration
    ...
    def _log_prob(self, sample, x_hat):
        ...
        #     aa = torch.sum(torch.sum(torch.sum(mask, 3), 2), 1)
        # # else:
        # #     aa = torch.sum(torch.sum(torch.sum(mask, 2), 1), 0)
    ...
    def forward(self, sample):
        ...
        # print(sample.dtype)
        # if sample.ndim ==4:
        #     sample = sample.reshape(args.batch_size, 1, args.L, args.L)
        # else:
        #     sample = sample.reshape(1, args.L, args.L)
        ...
        if self.z2:
            ...
            x_hat_inv2 = self._forward(x_flip)
            log_prob2 = self._log_prob(x_flip, x_hat_inv2)
            x_hat_inv3 = self._forward(x_rotate)
            log_prob3 = self._log_prob(x_rotate, x_hat_inv3)
            x_hat_inv4 = self._forward(x_flip_rotate)
    ...
    def log_prob(self, sample):
        ...
        #     sample = sample.reshape(1, args.L, args.L)
        x_hat = self._forward(sample)
        log_prob = self._log_prob(sample, x_hat.to(default_dtype_torch))
    ...
    def _log_prob2(self, sample, x_hat):
        mask = (sample + 1) / 2
        mask = mask.view(mask.shape[0], mask.shape[1], self.L, args.L1)  # My code
        log_prob = torch.log(x_hat + self.epsilon) * mask + torch.log(1 - x_hat + self.epsilon) * (1 - mask)
