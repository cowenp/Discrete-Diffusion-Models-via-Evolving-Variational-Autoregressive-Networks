...
def Test():
    ...
    args.beta = torch.tensor(1 / 4, dtype=torch.float32)
    ...
    args.net_depth = 4  # 3#3  # Depth of the neural network
    args.net_width = 16  # 128 # Width of the neural network
    args.batch_size = 300  # 1000 #batch size
    ...
    for lambda_tilt in lambda_tilt_Range:
        ...
        SampleSum = []
        if args.addBatch:
            label = torch.arange(0, args.size).long()  # torch.tensor([1:1:2])  # 2 shows the index
            num_class = args.size
        ...
        for Tstep in range(startT, args.Tstep):  # Time step of the dynamical equation
            ...
            with torch.no_grad():
                free_energy_mean3 = torch.mean(
                    torch.tensor(free_energy_mean3Temp, dtype=torch.float32).to(args.device)
                )  # loss.mean() #/ args.beta / args.size
                free_energy_mean3_2 = torch.mean(
                    torch.tensor(free_energy_mean3Temp2, dtype=torch.float32).to(args.device)
                )  # loss.mean() #/ args.beta / args.size
                net_new = copy.deepcopy(net)  # net
                net_new.requires_grad = False
                ...
                Loss1.append(
                    np.array(
                        torch.mean(torch.tensor(Loss1Temp, dtype=torch.float32).to(args.device))
                        .detach()
                        .cpu()
                    )
                )
                Loss1_2.append(
                    np.array(
                        torch.mean(torch.tensor(Loss1_2Temp, dtype=torch.float32).to(args.device))
                        .detach()
                        .cpu()
                    )
                )
